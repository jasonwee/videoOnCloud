...
public abstract class AbstractNioChannel {
	 ...
	 * 
	 * @param handler
	 *            which will handle events occurring on this channel.
	 * @param channel
	...
	/**
	 * The operation in which the channel is interested, this operation is provided
	 * to {@link Selector}.
	 * 
	...
	/**
	 * Reads the data using the key and returns the read data. The underlying
	 * channel should be fetched using {@link SelectionKey#channel()}.
	 * 
	 * @param key
	 *            the key on which read event occurred.
	 * @return data read.
	 * @throws IOException
	 *             if any I/O error occurs.
	 */
	...
	/*
	 * Called from the context of reactor thread when the key becomes writable. The
	 * channel writes the whole pending block of data at once.
	 */
	void flush(SelectionKey key) throws IOException {
		...
		while (true) {
			...
			if (pendingWrite == null) {
				// We don't have anything more to write so channel is interested in reading more
				// data
	...
	/**
	 * Queues the data for writing. The data is not guaranteed to be written on
	 * underlying channel when this method returns. It will be written when the
	 * channel is flushed.
	 * 
	 * <p>
	 * This method is used by the {@link ChannelHandler} to send reply back to the
	 * client. <br>
	 * Example:
	 * 
	 * <pre>
	 ...
	 * 
	 * @param data
	 *            the data to be written on underlying channel.
	 * @param key
	 *            the key which is writable.
	 */
