...
public class NioServerSocketChannel extends AbstractNioChannel {
	...
	/**
	 * Creates a {@link ServerSocketChannel} which will bind at provided port and
	 * use <code>handler</code> to handle incoming events on this channel.
	 * <p>
	 * Note the constructor does not bind the socket, {@link #bind()} method should
	 * be called for binding the socket.
	 * 
	 * @param port
	 *            the port on which channel will be bound to accept incoming
	 *            connection requests.
	 * @param handler
	...
	public int getInterestedOps() {
		// being a server socket channel it is interested in accepting connection from
		// remote peers.
	...
	/**
	 * Reads and returns {@link ByteBuffer} from the underlying
	 * {@link SocketChannel} represented by the <code>key</code>. Due to the fact
	 * that there is a dedicated channel for each client connection we don't need to
	 * store the sender.
	 */
	@Override
	...
	public void bind() throws IOException {
		...
		System.out.printf("Bound TCP socket at port: %s", port);
	}

	/**
	 * Writes the pending {@link ByteBuffer} to the underlying channel sending data
	 * to the intended receiver of the packet.
	 */
	@Override
